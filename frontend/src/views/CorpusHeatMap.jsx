...
import {
  Btn, Badge, Burst, Card, Display, Em, Eyebrow, GShape, HandPlayButton, Icon, Select,
    ...
    <Card padding={0} withGrain={false}>
      ...
      <div style={{ padding: 18 }}>
        <DrawerSnippetText text={text} editedSpan={spanRange}
          onSpanEdit={(cs, ce) => setSpanRange({ char_start: cs, char_end: ce })}/>
        {score !== undefined && (
        ...
        {snippet.has_audio && (
          <div style={{ marginTop: 14, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 12 }}>
            <HandPlayButton playing={playing} onClick={togglePlay} size={42} label="Play span"/>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-muted)', lineHeight: 1.4 }}>
              span {Math.round(selectedStart)}s → {Math.round(selectedEnd)}s
            </div>
            <audio ref={audioRef}
...
function DrawerSnippetText({ text, editedSpan, onSpanEdit }) {
  ...
  return (
    <div ref={containerRef}
      style={{ fontFamily: 'var(--font-display)', fontSize: 22, lineHeight: 1.35, color: 'var(--ink)', margin: 0, textWrap: 'pretty', userSelect: 'none' }}>
      {before}
      {onSpanEdit && (
