#!/usr/bin/env python3
"""
Ingest audio / Cortico JSON into a corpus.

Examples:
    # Audio folder:
    python scripts/ingest.py --corpus "my corpus" --audio-dir /path/to/wavs/

    # Cortico JSON folder (text-only):
    python scripts/ingest.py --corpus "bos" --cortico-dir ./cortico_api_transcripts_json/

    # Cortico + matching WAV folder (paired by stem):
    python scripts/ingest.py --corpus "bos" \\
        --cortico-dir ./cortico_api_transcripts_json/ --audio-dir /path/to/wavs/
"""
...
def main() -> int:
    ...
    ap.add_argument("--cortico-dir", help="Directory of Cortico transcript JSONs.")
    ap.add_argument("--language", default=None, help="ASR language (e.g. 'en').")
    args = ap.parse_args()

    if not args.audio_dir and not args.cortico_dir:
        ap.error("Provide --audio-dir and/or --cortico-dir")
    ...
    if args.cortico_dir:
        for p in sorted(Path(args.cortico_dir).glob("*.json")):
            stem = p.stem
            if stem in audio_by_stem:
                print(f"  ingesting {stem} (cortico + audio)")
                cid = ingest_cortico_with_audio(
                    corpus_id, stem, str(p), str(audio_by_stem.pop(stem))
                )
            else:
                print(f"  ingesting {stem} (cortico only)")
                cid = ingest_cortico_only(corpus_id, stem, str(p))
            print(f"    -> conversation id {cid}")

    # Any remaining audio files (no matching cortico JSON): ingest as audio-only.
